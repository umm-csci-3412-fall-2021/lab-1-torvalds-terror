...
for FILE in *.txt; do 
    awk 'match($0, /([a-zA-Z 0-9 0-9]+):[0-9]+:[0-9]+ [a-zA-Z_]+ sshd\[[0-9]+\]: Failed password for (|invalid user {1,})([a-zA-Z0-9]+) from ([0-9.0-9.0-9.0-9]+) port [0-9]+ ssh2/, groups) {print groups[1] groups[3] groups[4] "\n"}' < "$FILE" > failed_login_data.txt
done
