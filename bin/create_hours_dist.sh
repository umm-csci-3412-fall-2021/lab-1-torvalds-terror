...
cat ./*/failed_login_data.txt | awk 'match($0, /[a-zA-Z]+ [ 0-9]+ ([0-9]+) .+[ 0-9.]+/, groups) {print groups[1]}' | sort | uniq -c | awk 'match($0, /([0-9]+) ([ 0-9]+)/, groups) {print "data.addRow([\x27"groups[2]"\x27, "groups[1]"]);"}' > "$tmp"

#Back it up a step
cd "$cur_dir" || exit

#Calls wrap contents on the temp file just created and passes the necessary html components
./bin/wrap_contents.sh "$tmp" html_components/hours_dist "$dir"/hours_dist.html
