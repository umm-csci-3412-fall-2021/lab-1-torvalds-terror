...
join "$cur_dir"/sortedIPs.txt "$cur_dir"/etc/country_IP_map.txt | awk 'match($0, /[0-9.]+ ([a-zA-Z]+)/, groups) {print groups[1]}' | sort | uniq -c | awk 'match($0, /([0-9]+) ([a-zA-Z]+)/, groups) {print "data.addRow([\x27"groups[2]"\x27, "groups[1]"]);"}' >> tmpAddRows.txt

#Back it up a step
cd "$cur_dir" || exit

#Calls wrap contents on the temp file just created and passes the necessary html components
./bin/wrap_contents.sh "$dir"/tmpAddRows.txt html_components/country_dist "$dir"/country_dist.html